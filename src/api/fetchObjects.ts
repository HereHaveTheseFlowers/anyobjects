    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (data) {
        for (const objectpath of data.objectsList) {
          ...
          request.open(
            "GET",
            `${window.location.origin}/objects/${objectpath}/objectInfo.json`,
            true,
          );
          ...
          request.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
              ...
              jsonData["mainimage"] =
                `${window.location.origin}/objects/${objectpath}/main.png`;
              jsonData["previewimage"] =
                `${window.location.origin}/objects/${objectpath}/preview.png`;
