  ...
  function filterUpdate() {
    ...
    for (const object of objects) {
      ...
      if (!newfilter || newfilter === "ВСЁ") {
        ...
        element.dataset.category === newfilter
      ) {
        element.classList.remove("object-card_hidden");
      } else {
...
function ObjectCard(props: ObjectCardProps) {
  ...
  const navigateObject = () =>
    navigate(`${RouterList.OBJECT}/${props.objectkey}`);
    ...
    if (!props.previewimage || imageLoading) {
      return (
        <div className="object-card" data-category={category}>
          <img
            className="object-card__image image_visibility_hidden"
            onLoad={() => {
              handleImageLoaded();
            }}
            ...
            alt={props.alttext}
            draggable="false"
            width={objectImageSize}
            height={objectImageSize}
          />
          <div className="skeleton-image"></div>
          <div className="object-card__info" ref={cardInfoRef}>
            <span
              className="object-card__brand skeleton-box"
              style={{ width: "20%" }}
            />
            <span
              className="object-card__name skeleton-box"
              style={{ width: "40%" }}
            />
            <span
              className="object-card__price skeleton-box"
              style={{ width: "20%" }}
            />
          </div>
        </div>
      ...
      return (
        ...
        >
          <img
            className="object-card__image object-card__image_loaded"
            src={props.previewimage}
            alt={props.alttext}
            draggable="false"
            width={objectImageSize}
...
function ObjectCardAdmin(props: ObjectCardProps) {
  ...
  return (
    <div className="object-card object-card_mode_admin">
      <form
        className="object-card__info object-card__info_mode_admin"
        onSubmit={handleRefreshObject}
      >
        ...
        <div className="object-card__inputfield">
          ...
          <input
            ...
            type="text"
            className="object-card__input"
            defaultValue={props.additionalinfo.replaceAll("\n", "[ПЕРЕНОС]")}
          />
        </div>
        <div className="object-card__inputfield">
          ССЫЛКА:
        ...
        <button
          ...
          type="button"
          onClick={handleOpenDeleteObjectModal}
          aria-label="Удалить объект"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
            <path d="M0 13.63 5.63 8 0 2.37 2.37 0 8 5.63 13.63 0l2.34 2.37-5.6 5.6L16 13.63 13.63 16 8 10.34l-5.63 5.63Z" />
          </svg>
        </button>
      </form>
      <img
        className="object-card__image object-card__image_mode_admin"
        src={props.previewimage}
        alt={props.alttext}
        draggable="false"
        width={objectImageSize}
